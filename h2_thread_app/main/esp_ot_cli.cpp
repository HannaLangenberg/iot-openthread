...
#define REMOTE_PORT 18090   // not used
...
#define REMOTE_IPV4 "10.0.32.22"                            // IPv4 VM im IDiAL
...
/**
 * embedded IPv4 Adresse zum Thread-Interface hinzufügen, nur zu Testzwecken
 */
...
/**
 * Methode um ein simples Muster im WS2812-Streifen laufen zu lassen.
 */
...
void initCoapServer(otInstance *aInstance)
{
    ...
    // location.mUriPath = "set_location";          // set the location in IDiAL
...
/**
 * Send-Funktion inkl. embedded IPv4 um Nachrichten in ein WLAN ohne IPv6 zu schicken
 */
void send_coap_request_sensor_data_nat64(otInstance *instance, const char *message)
{
    ...
    otCoapMessageInit(request, OT_COAP_TYPE_NON_CONFIRMABLE, OT_COAP_CODE_POST); //OT_COAP_TYPE_CONFIRMABLE
    otCoapMessageGenerateToken(request, OT_COAP_DEFAULT_TOKEN_LENGTH);
    otCoapMessageAppendUriPathOptions(request, "sensor"); // alternativ otCoapMessageAppendProxyUriOption
...
static void sensor_data_loop(void *param) {
    ...
    //AddUnicastAddress(instance);  // nur testweise
