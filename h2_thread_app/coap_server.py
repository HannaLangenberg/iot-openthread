...
class BlockResource(resource.Resource):
    ...
    async def render_post(self, request):
        # Retrieve the payload from the POST request
        payload = request.payload
        print(f"Received POST request with payload: {payload.decode('utf-8')}")

        # Process the payload as necessary (e.g., logging or modifying it)
        # In this case, we'll just return an acknowledgment message
        response_payload = b"POST request received and processed."

        # Return the response as a POST acknowledgment
        return aiocoap.Message(code=aiocoap.CHANGED, payload=response_payload)


class SensorResource(resource.Resource):
    """
    Resource that handles PUT requests for the / endpoint.
    This resource will store the received payload and respond accordingly.
    """

    def __init__(self):
        super().__init__()
        self.data = b"Initial data"  # Default content for the resource

    async def render_get(self, request):
        """Handles GET requests. Returns the current data of the resource."""
        return aiocoap.Message(payload=self.data)

    async def render_put(self, request):
        """
        Handles PUT requests.
        The server stores the payload from the PUT request and acknowledges the update.
        """
        # Get the payload from the PUT request
        new_data = request.payload
        print(f"Received PUT request with payload: {new_data.decode('utf-8')}")

        # Store the new data (you can process it or save it as needed)
        self.data = new_data
